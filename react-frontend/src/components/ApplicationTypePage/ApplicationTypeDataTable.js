...
const ApplicationTypeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownArrayTemplate0 = (rowData, { rowIndex }) => <p >{rowData.name}</p>
const dropdownArrayTemplate1 = (rowData, { rowIndex }) => <p >{rowData.type}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "applicationType"});
...
<Column field="name" header="Name" body={dropdownArrayTemplate0} filter={selectedFilterFields.includes("name")} hidden={selectedHideFields?.includes("name")}  style={{ minWidth: "8rem" }} />
<Column field="type" header="Type" body={dropdownArrayTemplate1} filter={selectedFilterFields.includes("type")} hidden={selectedHideFields?.includes("type")}  style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload ApplicationType Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search ApplicationType" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ApplicationTypeDataTable;