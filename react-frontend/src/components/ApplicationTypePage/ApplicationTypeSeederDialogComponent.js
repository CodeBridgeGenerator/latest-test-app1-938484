...
import seederData from "./applicationType.seeder.json";
...
const ApplicationTypeSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("applicationType").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ApplicationTypeSeederDialogComponent);
