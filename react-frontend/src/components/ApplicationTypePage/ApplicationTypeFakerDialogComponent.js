...
import applicationTypeFakerFactory from "./applicationTypeFakerFactory";
...
const ApplicationTypeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = applicationTypeFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("applicationType").create(elem));
...
export default connect(mapState, mapDispatch)(ApplicationTypeFakerDialogComponent);
