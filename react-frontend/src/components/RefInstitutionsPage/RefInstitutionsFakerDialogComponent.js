...
import refInstitutionsFakerFactory from "./refInstitutionsFakerFactory";
...
const RefInstitutionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = refInstitutionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("refInstitutions").create(elem));
...
export default connect(mapState, mapDispatch)(RefInstitutionsFakerDialogComponent);
