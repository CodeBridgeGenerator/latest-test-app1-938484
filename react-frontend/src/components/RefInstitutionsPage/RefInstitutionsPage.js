...
import RefInstitutionsDatatable from "./RefInstitutionsDataTable";
...
import RefInstitutionsFakerDialogComponent from "./RefInstitutionsFakerDialogComponent";
import RefInstitutionsSeederDialogComponent from "./RefInstitutionsSeederDialogComponent";

const RefInstitutionsPage = (props) => {
    ...
    const filename = "refInstitutions.csv";
    ...
    const service = _.find(config.services, { serviceName : "refInstitutions"});
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("refInstitutions").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "RefInstitutions", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
        ...
        const promises = data.map((e) => client.service("refInstitutions").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "RefInstitutions",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/refInstitutions/${data._id}`);
    };
        ...
        <div className="mt-5">
            <div className="grid">
                <div className="col-6 flex justify-content-start">
                    <h3 className="mb-0 ml-2">RefInstitutions </h3>
                </div>
                <div className="col-6 flex justify-content-end">
                    <>
                    <Button label="add" loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="refInstitutions-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-12" role="refInstitutions-datatable">
                    <RefInstitutionsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>
            ...
            <RefInstitutionsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <RefInstitutionsCreateDialogComponent entity={newRecord} show={showCreateDialog} onHide={() => setShowCreateDialog(false)} onCreateResult={onCreateResult} />
            <RefInstitutionsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <RefInstitutionsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(RefInstitutionsPage);
