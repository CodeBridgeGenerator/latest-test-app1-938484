...
import seederData from "./refInstitutions.seeder.json";
...
const RefInstitutionsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("refInstitutions").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(RefInstitutionsSeederDialogComponent);
