...
const RefInstitutionsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.program}</p>
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.type}</p>
const pTemplate4 = (rowData, { rowIndex }) => <p >{rowData.address}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "refInstitutions"});
...
<Column field="program" header="Program" body={pTemplate2} filter={selectedFilterFields.includes("program")} hidden={selectedHideFields?.includes("program")}  sortable style={{ minWidth: "8rem" }} />
<Column field="type" header="Type" body={pTemplate3} filter={selectedFilterFields.includes("type")} hidden={selectedHideFields?.includes("type")}  sortable style={{ minWidth: "8rem" }} />
<Column field="address" header="Address" body={pTemplate4} filter={selectedFilterFields.includes("address")} hidden={selectedHideFields?.includes("address")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload RefInstitutions Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search RefInstitutions" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default RefInstitutionsDataTable;