...
const RefInstitutionsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,title: _entity?.title,program: _entity?.program,type: _entity?.type,address: _entity?.address,
        ...
        const result = await client.service("refInstitutions").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info RefInstitutions created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in RefInstitutions" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create RefInstitutions" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="refInstitutions-create-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                ...
                <InputText id="program" className="w-full mb-3 p-inputtext-sm" value={_entity?.program} onChange={(e) => setValByKey("program", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["program"]) ? (
              <p className="m-0" key="error-program">
                {error["program"]}
              </p>
            ) : null}
          </small>
...
export default connect(mapState, mapDispatch)(RefInstitutionsCreateDialogComponent);
