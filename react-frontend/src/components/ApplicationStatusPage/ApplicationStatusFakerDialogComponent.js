...
import applicationStatusFakerFactory from "./applicationStatusFakerFactory";
...
const ApplicationStatusFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [applicationIDIds, setapplicationIDIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("applications")
                        ...
                        .then((data) => {
                        setapplicationIDIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = applicationStatusFakerFactory(props.user,fakerCount,applicationIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("applicationStatus").create(elem));
...
export default connect(mapState, mapDispatch)(ApplicationStatusFakerDialogComponent);
