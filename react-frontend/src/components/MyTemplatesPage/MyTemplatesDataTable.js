...
const MyTemplatesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.field1}</p>
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.field2}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "myTemplates"});
...
<Column field="field1" header="Field1" body={pTemplate2} filter={selectedFilterFields.includes("field1")} hidden={selectedHideFields?.includes("field1")}  sortable style={{ minWidth: "8rem" }} />
<Column field="field2" header="Field2" body={pTemplate3} filter={selectedFilterFields.includes("field2")} hidden={selectedHideFields?.includes("field2")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload MyTemplates Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search MyTemplates" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default MyTemplatesDataTable;