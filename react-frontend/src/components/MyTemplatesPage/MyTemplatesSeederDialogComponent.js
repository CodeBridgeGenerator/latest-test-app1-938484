...
import seederData from "./myTemplates.seeder.json";
...
const MyTemplatesSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("myTemplates").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(MyTemplatesSeederDialogComponent);
