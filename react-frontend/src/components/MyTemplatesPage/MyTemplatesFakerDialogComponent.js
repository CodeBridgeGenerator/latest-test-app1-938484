...
import myTemplatesFakerFactory from "./myTemplatesFakerFactory";
...
const MyTemplatesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = myTemplatesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("myTemplates").create(elem));
...
export default connect(mapState, mapDispatch)(MyTemplatesFakerDialogComponent);
