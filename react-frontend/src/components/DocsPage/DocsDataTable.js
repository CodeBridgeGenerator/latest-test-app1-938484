...
const DocsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.docName}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.docFileName}</p>
const pTemplate3 = (rowData, { rowIndex }) => <p >{rowData.docURL}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "docs"});
...
<Column field="docName" header="DocName" body={pTemplate1} filter={selectedFilterFields.includes("docName")} hidden={selectedHideFields?.includes("docName")}  sortable style={{ minWidth: "8rem" }} />
<Column field="docFileName" header="DocFileName" body={pTemplate2} filter={selectedFilterFields.includes("docFileName")} hidden={selectedHideFields?.includes("docFileName")}  sortable style={{ minWidth: "8rem" }} />
<Column field="docURL" header="DocURL" body={pTemplate3} filter={selectedFilterFields.includes("docURL")} hidden={selectedHideFields?.includes("docURL")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Docs Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Docs" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
