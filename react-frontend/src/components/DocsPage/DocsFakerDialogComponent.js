...
import docsFakerFactory from "./docsFakerFactory";
...
const DocsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = docsFakerFactory(props.user,fakerCount,applicationIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("docs").create(elem));
...
export default connect(mapState, mapDispatch)(DocsFakerDialogComponent);
