...
const [applicationTypeIDIds, setapplicationTypeIDIds] = useState();
const [refInstitutionIDIds, setrefInstitutionIDIds] = useState();
    ...
    useEffect(() => {
        ...
        client.service("users")
                        ...
                        .then((data) => {
                        setmembersIds(data.data);
                        });
client.service("applicationType")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setapplicationTypeIDIds(data.data);
                        });
client.service("refInstitutions")
                        .find({ query: { $select: ["_id","name"] } })
                        .then((data) => {
                        setrefInstitutionIDIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = applicationsFakerFactory(props.user,fakerCount,membersIds,applicationTypeIDIds,refInstitutionIDIds);
